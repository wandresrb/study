...
Eso es todo: no hay `setup()`, ni plugin intermediario, ni orden de carga que respetar. Al abrir un `.c`, Neovim busca los servidores habilitados cuyo `filetypes` encaje, resuelve la raíz con `root_markers`, lanza el proceso y adjunta el cliente. Los atajos globales de 0.12 (`grn`, `gra`, `grr`, `K`, `]d` y `[d`) y los extras que montaste en la lección 3.1 sobre el evento `LspAttach` valen para clangd sin tocar nada. Recuerda añadir `"clangd"` a la lista de `vim.lsp.enable` de tu `init.lua` — o la línea suelta de arriba, que es equivalente.
...
La rama `main` es una reescritura incompatible: `nvim-treesitter.configs` y sus opciones (`ensure_installed`, `highlight`, `indent`, `incremental_selection`) ya no existen. Lo tienes explicado en las lecciones 2.2 y 3.3.
</Callout>

Aquí no hay nada nuevo que configurar, y eso es lo elegante del diseño que ya tienes montado. El parser de C entró en tu lista de `install()` en la lección 3.3 (además, C es uno de los seis que Neovim trae de serie), y el autocomando genérico de `lua/config/treesitter.lua` —el de la 2.2, sin lista de lenguajes— enciende resaltado, plegado e indentación en cuanto detecta que hay parser. Abrir un `.c` basta.

`install` es asíncrona. Si necesitas que el arranque de una máquina nueva no siga hasta tener un parser, encadena la espera: `require("nvim-treesitter").install({ "c" }):wait(300000)`.

Con el parser puesto, los objetos de texto de la lección 3.3 valen en C: `dif` vacía el cuerpo de una función, `vaf` la selecciona entera, `]f` salta a la siguiente y `<leader>cp` intercambia dos argumentos de una llamada.

## Formateo con clang-format

Ya configuraste `c = { "clang_format" }` en conform (lección 3.4). El binario `clang-format` viene con LLVM; instálalo con el gestor de paquetes del sistema o con Mason, que para esto es exactamente lo que necesitas. Personaliza el estilo con un `.clang-format` en la raíz:
...
`codelldb` es el adaptador para lenguajes nativos. Define una configuración de debug para C sobre el `nvim-dap` de la lección 3.4:
...
Y la parte que casi todo el mundo olvida: **cargar el módulo**. Ojo con el matiz según tu vía de instalación de `nvim-dap`. Con `vim.pack` el plugin ya está cargado, así que un `require` normal en el `init.lua` funciona. Con `lazy.nvim` lo instalaste diferido por `keys`, y un `require("dap")` en el arranque lo forzaría a cargar, tirando la carga perezosa a la basura — ahí el sitio correcto es dentro del `config` de la spec:

```lua title="init.lua (vía vim.pack)"
require("config.dap-c")
```

```lua title="lua/plugins/dap.lua (vía lazy.nvim: añade al config existente)"
config = function()
  -- ...lo que ya tenías de la lección 3.4...
  require("config.dap-c")
end,
```
...
<Challenge title="C con superpoderes">

...
El completado de clases aparece con el autocompletado, y el autocompletado es el dueño de <KeyCap keys="Ctrl-Space" /> y de <KeyCap keys="Tab" /> en todo este track — tanto si usas el nativo de 0.12 (`vim.o.autocomplete` más `vim.opt.completeopt`) como blink.cmp de la lección 3.2. Ningún plugin de esta lección ni de las siguientes puede reclamar esas dos teclas. Y `s` y `S` son de flash.nvim, con `mini.surround` en el prefijo `gs` (lección 3.3).
</Callout>
...
<Install repo="brenoprata10/nvim-highlight-colors" nota="Con render virtual pinta un cuadradito junto al valor; si prefieres teñir el fondo, usa render background. Necesita termguicolors.">
<Fragment slot="lazy">
...
El comodín `*` ya lo declaraste en la lección 3.1 (y quizá le sumaste las capabilities de blink.cmp, lección 3.2): añade esta línea a **esa misma tabla** en lugar de declarar el comodín dos veces, porque la fusión es superficial y la segunda declaración pisaría la primera.
</Callout>
...
2. **`eslint_d` por nvim-lint** (lección 3.4): más ligero, corre el linter al guardar y ya lo tienes montado.
...
Si seguiste la lección 3.3, todo el stack web ya está en tu lista de `install()` — la llamada es idempotente y no cuesta nada dejarlo explícito. El resto no requiere nada: el autocomando genérico de `lua/config/treesitter.lua` (lección 2.2) detecta el parser al abrir el archivo y enciende resaltado, plegado e indentación, sin listas por lenguaje.
...
El cierre y renombrado automático de etiquetas ya lo instalaste en la lección 3.3 con `nvim-ts-autotag`: escribes `<div>` y se cierra solo; renombras la etiqueta de apertura y la de cierre te sigue. En React eso son cientos de pulsaciones al día.

## package.json vivo

<Install repo="vuki656/package-info.nvim" nota="Necesita nui.nvim y npm en el sistema. Detecta solo si el proyecto usa npm, yarn, pnpm o bun, incluso dentro de un monorepo.">
<Fragment slot="lazy">
...
<Challenge title="Monta un .tsx real">

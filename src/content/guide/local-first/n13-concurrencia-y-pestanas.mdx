...
<Callout type="info" title="Una puerta entreabierta, y solo en Chrome">
La frase anterior describe la línea base multiplataforma, y contra ella hay que diseñar. Pero conviene saber que Chrome 121 entreabrió la puerta: `createSyncAccessHandle({ mode: 'read-only' })` permite varios lectores simultáneos mientras nadie escriba, y `mode: 'readwrite-unsafe'` permite varios escritores **sin cerrojo** — el navegador deja de arbitrar y dos motores con cachés independientes pueden corromper la base exactamente como describe esta lección. A mediados de 2026 el parámetro es experimental, está fuera del standard track y no existe en Firefox ni en Safari: ninguna arquitectura seria puede asumirlo, y como mucho sirve de atajo con plan alternativo obligatorio.
</Callout>
...
<Challenge title="Rompe tu aplicación con dos pestañas">

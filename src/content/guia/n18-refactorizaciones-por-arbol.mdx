...
- Intercambiar parámetros y hermanos sintácticos con las operaciones de `swap`.
...
local parser = assert(vim.treesitter.get_parser(buf, lang))
...
local function renombrar_local(nuevo)
  ...
  local rangos = {}
  for id, n in q:iter_captures(root, buf, 0, -1) do

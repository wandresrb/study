...
La regla del track es sencilla y la vas a agradecer: `Ctrl-Espacio` y `Tab` son intocables, los **saltos estructurales** van en los prefijos `]` y `[` que Vim reserva precisamente para eso, y las **acciones** van detrás de `<leader>`. Tampoco se ceden `s` ni `S`: son comandos nativos de Vim y además `s` es el prefijo de operador de `mini.surround`, con `sa`, `sd` y `sr`. Si un plugin te pide una de estas teclas, la respuesta es cambiar la tecla, no el hábito.
</Callout>
...
-- Mismas teclas que la version nativa. Nunca Ctrl-Espacio ni Tab.

...
description: "Desde que abres un buffer hasta que un servidor responde: resolución de la raíz con root_dir y root_markers, reutilización de procesos, adjuntar y desadjuntar clientes, el evento LspAttach como único punto de enganche fiable y la convivencia de varios servidores sobre el mismo fichero."
subject: neovim
level: 20
order: 2
icon: "20.2"
...
vim.api.nvim_create_autocmd("LspAttach", {
  callback = function(args)
    ...
    map("grr", vim.lsp.buf.references, "referencias")
    map("gri", vim.lsp.buf.implementation, "implementaciones")
...
vim.api.nvim_create_user_command("LspReiniciar", function()
  ...
  for _, c in ipairs(vim.lsp.get_clients({ bufnr = buf })) do
    nombres[#nombres + 1] = c.name
    c:stop()
  end
  vim.defer_fn(function() vim.lsp.enable(nombres) end, 500)
end, { desc = "reinicia los clientes del buffer" })
```
...
3. Mueve todos tus atajos de LSP a un único `LspAttach` y verifica que ya no existen en buffers sin servidor.
4. Añade un `LspDetach` que registre cuándo un cliente abandona un buffer, y provócalo parando el servidor a mano.

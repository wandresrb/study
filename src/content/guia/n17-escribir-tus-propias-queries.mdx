...
<Callout type="warn" title="La modeline va en la primerísima línea">
`;; extends` debe ser la primera línea útil del fichero, con dos puntos y coma, y no admite nada delante. Un comentario de cortesía por encima la invalida silenciosamente: no hay error, simplemente el fichero pasa a comportarse como sustituto y tu resaltado desaparece. Es el fallo número uno de quien empieza.
</Callout>
...
-- recomponer la query: primero lo que aporta el runtimepath, luego lo tuyo
local partes = {}
for _, fichero in ipairs(vim.treesitter.query.get_files("lua", "highlights")) do
  table.insert(partes, table.concat(vim.fn.readfile(fichero), "\n"))
end
table.insert(partes, extra)

vim.treesitter.query.set("lua", "highlights", table.concat(partes, "\n"))
```

## El orden de precedencia

Con varios ficheros aportando patrones, es inevitable que dos capturas cubran el mismo rango. La resolución sigue una jerarquía de tres niveles que conviene tener grabada, de la más fuerte a la más débil.

El nivel decisivo es la **prioridad explícita**. Un patrón con `#set! priority` gana sobre cualquier otro de prioridad inferior, sin importar dónde esté el fichero ni en qué orden se concatenó. La prioridad por defecto de un resaltado de Treesitter es 100; los diagnósticos y otras fuentes de extmarks usan valores propios, por lo que subir a 105 o 110 basta para imponerse a los patrones genéricos sin pisar la señalización de errores.

El segundo nivel es la **especificidad del rango**: cuando dos capturas de igual prioridad se solapan, el rango más pequeño acaba visible sobre el más grande, que es exactamente lo que quieres cuando pintas un token concreto dentro de una expresión entera.

El tercer nivel, y el único que no deberías usar como mecanismo de diseño, es el **orden de aparición**: entre capturas de igual prioridad y mismo rango, la que se aplica después queda encima. Depender de esto es frágil, porque ese orden es un efecto colateral de cómo se concatenaron ficheros que no controlas. Si el resultado te importa, dilo con `#set! priority` y deja de rezar.
...
; apagar un resaltado sin borrar el patron ajeno
((comment) @none
  (#lua-match? @none "^%-%-%-@"))
```
...
<Reto title="Construye tu capa de resaltado">
1. Crea `~/.config/nvim/after/queries/lua/highlights.scm` con la modeline `extends` y un patrón propio.
2. Borra deliberadamente la modeline, reinicia y observa cómo el resaltado del lenguaje se derrumba; restáurala.
3. Añade dos patrones que compitan por el mismo rango y decide el ganador únicamente con `#set! priority`.
4. Usa `@none` para apagar el resaltado de una construcción concreta sin tocar el fichero del plugin.
5. Crea un fichero con `inherits` para un dialecto y comprueba con `:Inspect` que hereda los colores del lenguaje base.
6. Ejecuta `vim.treesitter.query.get_files` antes y después de tus cambios y explica la diferencia en la lista.
</Reto>

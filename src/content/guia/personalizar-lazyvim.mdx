...
En una LazyVim recién instalada, `:map s` te responde que `s` y `S` son de **flash.nvim**, y `:map <C-Space>` te dice que esa tecla también, porque flash la usa para imitar la vieja selección incremental de Treesitter. Ahí tienes dos problemas de distinta gravedad.

### Problema 1: `<C-Space>` no es de nadie más

En este track `<C-Space>` está **reservado para el autocompletado** —el nativo de 0.12 se enciende con `vim.o.autocomplete` y se afina con `vim.opt.completeopt`— y `<Tab>` para moverse por su menú. Si flash se queda con `<C-Space>`, tu completado deja de responder y tardarás una tarde en descubrir por qué. Este no se negocia: se quita.

### Problema 2: `s` tiene tres dueños

`s` y `S` son **verbos nativos de Vim** (`s` sustituye el carácter bajo el cursor, `S` la línea entera). Y en cuanto instalas `mini.surround`, `s` es además su **prefijo de operador**: `sa` para añadir delimitadores, `sd` para borrarlos, `sr` para cambiarlos. La documentación de mini.surround añade un detalle que lo empeora: cuando sus mapeos empiezan por `s`, el plugin mapea la `s` suelta a `<Nop>` para que no se dispare el `s` nativo si tardas en pulsar la segunda tecla.

Resultado: gana quien cargue el último. Si gana surround, flash deja de saltar; si gana flash, escribes `saiw"` y Vim se queda esperando un salto que nunca llega. Y como depende del orden de carga, **falla de forma intermitente**, que es la peor forma de fallar.

<Callout type="info" title="Qué hace LazyVim con esto">
LazyVim no deja los dos por defecto: en su extra `coding.mini-surround` mueve surround al prefijo `gs` (`gsa`, `gsd`, `gsr`) precisamente porque `s` ya es de flash. Así que en una LazyVim de serie no chocan. El choque aparece cuando **tú** añades `mini.surround` con sus valores por defecto encima —por ejemplo trayendo tu archivo del Nivel 2— y de pronto hay dos plugins peleando por la misma tecla.
</Callout>

### Las dos salidas: elige una, no las dos

**Opción A — mueves flash y te quedas la `s`.** Es lo coherente con el resto del track. Pasa `keys` como función: recibes las de LazyVim y devuelves solo las tuyas, así que las suyas desaparecen.
...
Ahora `s` y `S` vuelven a ser de Vim, y `mini.surround` puede usar sus `sa`, `sd`, `sr` por defecto sin pelearse con nadie.

**Opción B — dejas flash en `s` y mueves surround.** Cambias el prefijo con la opción `mappings` del propio plugin:
...
Si eliges esta, el extra `coding.mini-surround` de LazyVim ya te la da hecha: actívalo con `:LazyExtras` y no escribas nada.

**Y en cualquiera de los dos casos, recupera `<C-Space>`:**
...
Reinicia y vuelve a ejecutar `:map s` y `:map <C-Space>`. `:map s` debe mostrar **un solo dueño** —flash o surround, el que hayas elegido— y `:map <C-Space>` debe salir vacío en modo normal. Si `:map s` te lista dos entradas, todavía tienes la bomba puesta: solo que hoy ha cargado en el orden que te favorece.
</Callout>

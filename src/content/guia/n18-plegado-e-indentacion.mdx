...
<Instalar repo="nvim-treesitter/nvim-treesitter" motivo="Solo vim.pack: la rama main de nvim-treesitter NO admite carga perezosa, asi que lazy.nvim no aporta nada aqui. Su spec en lazy seria la misma linea con branch = main, lazy = false y build = :TSUpdate, es decir, usar lazy para desactivar lo unico que lazy hace. Si ya usas lazy para otros plugins, puedes declararlo ahi igualmente; el resultado es identico." nota="El plegado y el resaltado son de Neovim; el plugin aporta los parsers, las queries y la funcion de indentacion. main no admite carga perezosa.">
<Fragment slot="pack">

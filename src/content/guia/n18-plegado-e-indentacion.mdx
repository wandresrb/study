...
- Activar el `foldexpr` nativo de Treesitter y controlar `foldlevel`, `foldtext` y la persistencia de pliegues.
- Leer una query de `folds` y añadir capturas propias para plegar lo que tu lenguaje no pliega por defecto.
- Configurar `indentexpr` por árbol y entender las capturas de `indents` y sus directivas.
...
vim.api.nvim_create_autocmd("FileType", {
  callback = function(args)
    ...
    vim.wo.foldlevel = 99        -- abre todo al entrar
  end,
})
...
local q = vim.treesitter.query.get(vim.bo.filetype, "folds")
print(q and table.concat(q.captures, ", ") or "sin query de folds")

-- Equivalente de lo que contiene un folds.scm tipico:
...
```lua
-- Lo que contiene un indents.scm, expresado como cadena para poder leerlo aqui:
local ejemplo = [[
  (function_declaration) @indent.begin      ; abre un nivel
  (block)                @indent.begin
  "end"                  @indent.branch     ; alinea con quien abrio
  (else_statement)       @indent.branch
  (comment)              @indent.ignore     ; no toques su sangria
  (preproc_directive)    @indent.zero       ; siempre en la columna uno
]]
```

El fichero `indents.scm` no captura "lo que se indenta" sino los **eventos** que modifican la sangría, y ahí está la elegancia del modelo. Una captura de comienzo indica que a partir de ese nodo se abre un nivel; una de fin lo cierra; una de reducción baja la línea actual sin afectar a las siguientes, que es justo lo que necesita un `else` o un `case`; una de rama marca los puntos donde una estructura se bifurca sin cambiar de profundidad; una de alineación pide que los elementos se alineen con el delimitador de apertura en lugar de con un múltiplo del ancho de tabulación; una de nivel cero fuerza la columna uno, como en las directivas de preprocesador; y una de ignorar excluye regiones que no deben tocarse, como el interior de una cadena multilínea. Además hay directivas que ajustan el momento en que la regla se aplica, porque no es lo mismo indentar una línea ya escrita que indentarla en el instante en que pulsas `Enter` y el nodo aún no existe.

Ese último punto explica la rareza más frecuente: mientras escribes, la línea está incompleta y el parser produce un nodo de error, de modo que la regla que debía dispararse todavía no puede emparejar. Por eso algunas capturas se marcan como inmediatas y por eso la indentación al teclear a veces difiere de la que obtienes al reindentar después con `=`. No es un fallo: es la diferencia entre indentar código sintácticamente válido e indentar código a medio nacer.
...
Hay además un detalle de configuración que provoca desconciertos evitables: `indentexpr` solo actúa si la opción que activa la indentación automática está encendida, y algunos ficheros de tipo la apagan. Cuando la sangría automática "no funciona" pese a estar bien configurado el árbol, la causa suele ser esa, no la query.

...
vim.keymap.set("n", "<leader>tq", ":EditQuery<CR>", { desc = "Editor de queries" })
...
**Predicado desconocido.** Estás usando un predicado que existe en un plugin pero no en el núcleo, o al revés. La solución es consultar `:help treesitter-predicates` y, si de verdad lo necesitas, registrarlo tú.
...
Las queries se compilan y se memorizan la primera vez que se piden. Editar un fichero `.scm` y volver al búfer no basta: seguirás viendo la versión anterior y creerás que tu cambio no hace nada. Durante la iteración usa `:EditQuery`, que no pasa por la caché, y reserva el reinicio del editor para la comprobación final. Es la explicación de la inmensa mayoría de los cambios que aparentemente no surten efecto.
</Callout>
...
local raiz = vim.treesitter.get_parser(0, "lua"):parse()[1]:root()
...
3. Provoca a propósito los cuatro errores de la sección y anota el mensaje exacto que devuelve cada uno.
...
5. Edita un fichero `.scm` sin reiniciar, comprueba que el cambio no se aplica y confirma que la caché era la culpable.

...
Neovim no siempre parsea el buffer entero. Por omisión, el resaltado pide solo el rango visible, y el resto del árbol se completa cuando hace falta. Desde `Neovim 0.11` el parseo puede además ser asíncrono, de modo que un archivo enorme no bloquea la interfaz mientras se reconoce.

```lua
local parser = vim.treesitter.get_parser(0)
parser:parse()          -- rango implicito, tipicamente lo visible
parser:parse(true)      -- forzar el buffer entero
parser:parse(true, function(_, arboles)
  vim.notify("arbol listo: " .. #arboles)
end)                    -- asincrono, con callback
```
</Callout>
...
local lt = vim.treesitter.get_parser(0)
lt:for_each_tree(function(arbol, hijo)
  local r = arbol:root()
  print(hijo:lang(), r:start(), r:end_())
end)
```
...
<Reto title="Medir la incrementalidad">

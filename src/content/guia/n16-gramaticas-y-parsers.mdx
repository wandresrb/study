...
La rama `main` de `nvim-treesitter` es **una reescritura incompatible**: trátala como otro plugin. El módulo `nvim-treesitter.configs` ya no existe, y con él desaparecieron `ensure_installed`, `highlight`, `indent` e `incremental_selection`. La rama `master` sigue publicada pero está congelada y solo sirve para Neovim 0.11. Si un tutorial te enseña `require("nvim-treesitter.configs").setup({ ... })`, está escrito para la rama muerta.
...
<Instalar repo="nvim-treesitter/nvim-treesitter" motivo="Solo vim.pack: la rama main de nvim-treesitter NO admite carga perezosa, asi que lazy.nvim no aporta nada aqui. Su spec en lazy seria la misma linea con branch = main, lazy = false y build = :TSUpdate, es decir, usar lazy para desactivar lo unico que lazy hace. Si ya usas lazy para otros plugins, puedes declararlo ahi igualmente; el resultado es identico." nota="main no admite carga perezosa: con lazy.nvim va lazy = false y build = ':TSUpdate'. install es asincrono; para un arranque sincrono encadena :wait(300000).">
<Fragment slot="pack">

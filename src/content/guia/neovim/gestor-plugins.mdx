...
<Pipeline
  titulo="el falso dilema: los dos acaban en el mismo sitio"
  resultado="runtimepath — de donde Neovim carga TODO"
  grupos={[
    { titulo: 'vim.pack — el nativo', tono: 'green', pasos: [
      { t: 'instala y fija versiones', d: 'Clona con git en site/pack/core/opt y mantiene su lockfile.' },
      { t: 'el tema, treesitter, las librerías', d: 'Lo que siempre está: sin specs, sin capas.' },
    ]},
    { titulo: 'lazy.nvim — la carga perezosa', tono: 'mauve', pasos: [
      { t: 'instala Y decide CUÁNDO cargar', d: 'event, ft, cmd, keys: no cargar hasta que pase algo.' },
      { t: 'lo que gana con diferirse', d: 'El depurador, los plugins por tipo de fichero, lo pesado de arranque.' },
    ]},
  ]}
/>
...
-- el editor. Si te pasa alguna vez, zR lo abre todo. Lección 1.7.
...
Fíjate en que no hay ninguna lista de lenguajes: el autocomando pregunta si existe parser y actúa en consecuencia. Instala un parser nuevo y funciona solo, sin tocar esto. El plegado que acabas de encender es el que dominaste en la lección 1.7: `za` para abrir y cerrar, `zR` para abrirlo todo — solo que ahora los pliegues los deduce el árbol de sintaxis.

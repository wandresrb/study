...
<Pipeline
  titulo="la conversación cliente ↔ servidor"
  resultado="el editor entiende tu código sin saber nada del lenguaje"
  grupos={[
    { titulo: 'Neovim pregunta', tono: 'blue', pasos: [
      { t: 'textDocument/definition', d: '«¿dónde se define el símbolo bajo el cursor?» — es lo que dispara CTRL-] o grt.' },
      { t: 'textDocument/completion', d: '«¿qué encaja aquí?» — alimenta el autocompletado de la próxima lección.' },
      { t: 'textDocument/rename', d: '«renombra este símbolo en todo el proyecto» — grn. El servidor devuelve las ediciones; Neovim las aplica.' },
    ] },
    { titulo: 'el servidor responde (y avisa solo)', tono: 'mauve', pasos: [
      { t: 'posiciones, listas, ediciones', d: 'Respuestas en JSON con coordenadas de fichero. Neovim las convierte en saltos, menús y cambios.' },
      { t: 'publishDiagnostics', d: 'Sin que nadie pregunte: cada vez que reanaliza, empuja la lista de errores y avisos del fichero.' },
    ] },
  ]}
/>
...
```lua title="init.lua"
-- Una sola línea decide qué se activa: los nombres de las fichas que quieres vivas.
vim.lsp.enable({
...
La lista crecerá conforme avances por el nivel: cada lección de lenguaje (3.5 a 3.8) te dirá qué nombre añadir aquí — o por qué **no** añadirlo, que también pasa. TypeScript, por ejemplo, tiene dos servidores rivales y la lección 3.8 te hace elegir uno; Rust y Swift ni siquiera pasan por esta lista.
...
No habilites `rust_analyzer` aquí si usas **rustaceanvim** (lección 3.6): ese plugin arranca y gestiona el servidor por su cuenta y tendrías dos clientes peleándose. Y `sourcekit-lsp` viene con Xcode, no hay nada que descargar (lección 3.7).
</Callout>
...
Con eso claro, lo que queda por mapear es poco. Los atajos que dependen del LSP viven en el evento `LspAttach`, que es el único momento en que existen buffer y cliente. Todo lo de esta sección y la siguiente va en un módulo nuevo, `lua/config/lsp.lua` — y un módulo en `lua/` no se ejecuta solo: cárgalo desde tu `init.lua`, junto a los `require` que ya tienes de la lección 2.1:

```lua title="init.lua"
require("config.lsp")   -- autocmd de LspAttach + estilo de diagnósticos
```
